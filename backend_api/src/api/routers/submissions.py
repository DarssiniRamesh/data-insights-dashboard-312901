...
from src.schemas import (
    CreateSubmissionRequest, CreateSubmissionResponse,
    GetSubmissionResponse, ApproveSubmissionRequest, ApproveSubmissionResponse,
    TriggerValidationRequest, TriggerValidationResponse,
    ErrorResponse
)
from src.services.submission import SubmissionService
from src.services.validation import ValidationService
from src.services.approval import ApprovalService, ApprovalError, AuthenticationError, AuthorizationError, SoDViolationError, SignatureError, InvalidStateError
...
async def create_submission(
    ...
    This endpoint transitions a draft to submission state and automatically queues
    a validation pipeline job.
    
    Authentication: Bearer token required
    Authorization: publisher, steward roles
    
    Args:
        request: Submission creation request with draft_id and audit context
        credentials: HTTP authorization credentials
        
    Returns:
        CreateSubmissionResponse with submission details and queued validation job
        
    Raises:
        HTTPException: 401 if auth fails, 400 if draft not found, 500 on error
    """
    db = get_connection()
...
async def get_submission(
    ...
    try:
        ...
        # Get latest validation run if exists
...
async def trigger_validation(
    ...
    This endpoint executes quality gates and generates a validation report.
    
    Authentication: Bearer token required
    Authorization: publisher, steward, system roles
    
    Args:
        submission_id: Submission to validate
        request: Validation trigger request with profile and audit context
        credentials: HTTP authorization credentials
        
    Returns:
        TriggerValidationResponse with pipeline_job_id and state
        
    Raises:
        HTTPException: 401 if auth fails, 404 if not found, 400 if invalid state
    """
    db = get_connection()
    auth_service = AuthService(db)
    ...
    try:
        ...
        # Create pipeline job record for the validation
...
async def approve_submission(
    ...
    - Electronic signature validation with timestamp window
    - Precondition checks (e.g., validation must pass)
    - State machine validation (must be in 'in_review' state)
    - Evidence package creation on approval
    
    Authentication: Bearer token required
    Authorization: steward, governance_admin roles only
    
    Args:
        submission_id: Submission to approve/reject
        request: Approval request with decision, signature, preconditions, and audit context
        credentials: HTTP authorization credentials
        
    Returns:
        ApproveSubmissionResponse with new state, published metadata, and evidence_package_id
        
    Raises:
        HTTPException: 401 if auth fails, 403 if SoD violated, 409 if signature/state invalid
    """
    db = get_connection()
    auth_service = AuthService(db)
