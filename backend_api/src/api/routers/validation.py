...
async def get_validation_report(
    ...
    try:
        ...
        if not report:
            raise HTTPException(
                status_code=404,
                detail=make_error_response("NOT_FOUND", f"Validation run {validation_run_id} not found", generate_id("req")),
            )
        ...
        raise HTTPException(
            status_code=500,
            detail=make_error_response("INTERNAL_ERROR", f"Failed to get validation report: {str(e)}", generate_id("req")),
        )
