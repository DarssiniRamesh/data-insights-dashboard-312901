...
class EvidenceService:
    ...
    Features:
    - Capture evidence payloads (JSON) with content hashing
    - Link evidence to workflow entities (drafts, submissions, approvals)
    - Store evidence in SQLite with artifact storage
    - Integrate with audit trail service
    - Provide create/read/list operations
    """
    ...
    def create_evidence_package(
        ...
        Args:
            ...
            evidence_items: List of evidence items with type, content, and metadata
            actor_user_id: User creating evidence
            actor_role: Role of actor
            correlation_id: Request correlation ID
            
        Returns:
            Dict with evidence_package_id, minted_identifier, and artifacts
        ...
        # Mint permanent identifier for evidence package
        minted_identifier = f"urn:evidence:{package_id}:{package_version}:{evidence_package_id}"
        
        # Create artifacts for each evidence item
        artifacts = []
        ...
        try:
            ...
            self.db.execute(
                ...
                """,
                (manifest_artifact_id, "evidence_manifest", "application/json", 
                 manifest_storage_ref, manifest_hash, created_at_utc)
            )
            
            # Insert evidence package record
            self.db.execute(
                """
            ...
            raise EvidenceError(
                ...
                f"Failed to create evidence package: {str(e)}",
    ...
    def get_evidence_package(self, evidence_package_id: str) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Evidence package dict with manifest or None if not found
        """
        cursor = self.db.execute(
            """
        ...
        if os.path.exists(manifest_storage_ref):
            ...
            if computed_hash != package["manifest_hash"]:
                raise EvidenceError(
                    ...
                    {
                        "evidence_package_id": evidence_package_id,
                        "expected_hash": package["manifest_hash"],
    ...
    def list_evidence_packages(self, submission_id: str) -> List[Dict[str, Any]]:
        """
        PUBLIC_INTERFACE
        List evidence packages for a submission.
        
        Args:
            submission_id: Submission identifier
            
        Returns:
            List of evidence package summaries
        """
        cursor = self.db.execute(
            """
            SELECT ep.evidence_package_id, ep.package_id, ep.package_version,
                   ep.minted_identifier, ep.created_at_utc, a.hash_sha256 as manifest_hash
            FROM evidence_packages ep
            JOIN artifacts a ON ep.manifest_artifact_id = a.artifact_id
            WHERE ep.submission_id = ?
            ORDER BY ep.created_at_utc DESC
            """,
            (submission_id,)
        )
    ...
    def get_evidence_artifact(self, artifact_id: str) -> Optional[Dict[str, Any]]:
        ...
        Raises:
            EvidenceError: If artifact integrity check fails
        """
        cursor = self.db.execute(
    ...
    def link_evidence_to_approval(
        ...
        Args:
            ...
            actor_user_id: User creating link
            actor_role: Role of actor
            correlation_id: Request correlation ID
        """
        # Verify both records exist
        cursor = self.db.execute(
            "SELECT evidence_package_id FROM evidence_packages WHERE evidence_package_id = ?",
            (evidence_package_id,)
        )
        ...
        cursor = self.db.execute(
            "SELECT approval_record_id FROM approval_records WHERE approval_record_id = ?",
            (approval_record_id,)
        )
        if not cursor.fetchone():
            raise EvidenceError(
                "APPROVAL_NOT_FOUND",
                f"Approval record {approval_record_id} not found"
            )
    ...
    def capture_validation_evidence(
        ...
        """
        PUBLIC_INTERFACE
        Capture validation report as evidence artifact.
        
        This is a helper method used by validation service to create evidence
        artifacts for validation reports.
        
        Args:
            validation_run_id: Validation run identifier
            submission_id: Submission identifier
            validation_report: Validation report content
            
        Returns:
            artifact_id of created evidence artifact
        """
        artifact_id = generate_id("art")
        created_at_utc = utc_now_iso()
        
        # Compute hash
        report_hash = compute_hash(validation_report)
        ...
        with open(storage_ref, 'w') as f:
            f.write(canonical_json(validation_report))
    ...
    def capture_approval_evidence(
        ...
        """
        PUBLIC_INTERFACE
        Capture approval record as evidence artifact.
        
        This is a helper method used by approval service to create evidence
        artifacts for approval records.
        
        Args:
            approval_record_id: Approval record identifier
            submission_id: Submission identifier
            approval_details: Approval details including signature
        ...
        # Compute hash
        approval_hash = compute_hash(approval_details)
        
        # Store artifact
        storage_ref = f"{self.artifact_root}/approval-records/{submission_id}/{approval_record_id}.json"
        Path(storage_ref).parent.mkdir(parents=True, exist_ok=True)
        
        with open(storage_ref, 'w') as f:
            f.write(canonical_json(approval_details))
        ...
        self.db.execute(
            ...
            """,
            (artifact_id, "approval_record", "application/json", storage_ref, approval_hash, created_at_utc)
        )
        
        return artifact_id
    
    def verify_evidence_integrity(self, evidence_package_id: str) -> Dict[str, Any]:
        """
        PUBLIC_INTERFACE
        Verify integrity of all artifacts in an evidence package.
        ...
        Raises:
            EvidenceError: If evidence package not found
        """
        # Get evidence package
        package = self.get_evidence_package(evidence_package_id)
        ...
        for artifact_info in artifacts:
            ...
            try:
                # This will verify hash internally
                artifact = self.get_evidence_artifact(artifact_id)
